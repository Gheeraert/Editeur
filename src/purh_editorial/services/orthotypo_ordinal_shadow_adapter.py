...
class OrthotypoOrdinalShadowError(ValueError):
    """Le parcours shadow des ordinaux ne peut pas être construit."""


@dataclass(slots=True)
class OrthotypoOrdinalShadowResult:
    rule_id: str
...
@dataclass(slots=True)
class OrthotypoOrdinalShadowAdapter:
    """Observe les ordinaux sans exécuter l'action native."""

    legacy_service: Any = field(default_factory=OrthotypoService)
    native_rule: Any = field(default_factory=OrdinalAbbreviationRule)
    engine: Any = field(
    ...
    def run(self, document: Document) -> OrthotypoOrdinalShadowResult:
        ...
        if (
            ...
            raise OrthotypoOrdinalShadowError(
        ...
        try:
            ...
            raise OrthotypoOrdinalShadowError(str(exc)) from exc
        ...
        for sequence, target in enumerate(targets):
            ...
            observation = build_legacy_text_observation(
                ...
                failure_justification=(
                    "Une transformation legacy d’ordinaux n’a pas pu être "
                    "convertie."
                ),
            )
        ...
        return OrthotypoOrdinalShadowResult(
            rule_id=RULE_ID,
