> **Statut : architecture legacy, non utilisée par le point d'entrée actuel (`main.py`).**
> Ce document décrit l'architecture « pivot Python‑JSON / scoring / seuils / IA multi-niveaux / exports multiples » construite avant le redémarrage `reborn`. Elle reste présente dans le dépôt (`pipeline/step1.py`, `services/orthotypo_service.py`, `services/structure_service.py`, `ui/step1_dialog.py`, etc.) mais n'est plus branchée sur `main.py` depuis le passage à l'UI Tkinter minimale du correcteur `reborn`.
>
> La stratégie actuelle est décrite dans [`docs/REBORN_ARCHITECTURE.md`](../REBORN_ARCHITECTURE.md). Ce document est conservé pour référence historique et pour l'éventuelle récupération de code (règles, garde-fous, fixtures) — voir `docs/REBORN_ARCHITECTURE.md` §10 « Politique de réutilisation ».

# PURH Editorial Studio — architecture pivot (legacy)
...
## 8. Lancement (voie legacy — non branchée sur `main.py`)

```bash
python -m venv .venv
.venv\Scripts\activate
python -m pip install -r requirements.txt
python -c "from purh_editorial.ui import run_step1_dialog; run_step1_dialog()"
```
...
- [Contrat du pivot Python‑JSON](PIVOT_JSON_CONTRACT.md)
- [Contrat des exporteurs](EXPORTERS_CONTRACT.md)
- [Modèle de décision éditoriale](EDITORIAL_DECISION_MODEL.md)
- [Pipeline éditorial](EDITORIAL_PIPELINE.md)
- [Politique IA](AI_STYLE_POLICY_V1.md)
- [Stratégie de test](TEST_STRATEGY.md)
- [Fixtures](FIXTURES.md)
- [Mapping Métopes](METOPES_MAPPING.md)
- [Audit documentaire pivot](../journal/DOCS_AUDIT_PIVOT.md)
