> **Statut : décrit le code couleur de l'architecture legacy (8 teintes, suggestions IA, structure heuristique), pas celui du correcteur `reborn` actuel.**
> Le correcteur `reborn` (`corrector/word_document.py`) n'utilise aujourd'hui que deux couleurs : jaune pour les corrections appliquées automatiquement, turquoise pour les diagnostics signalés sans modification du texte. Voir [`docs/NOTICE_COULEURS_WORD.md`](../NOTICE_COULEURS_WORD.md) pour la notice à jour.
