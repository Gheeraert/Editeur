...
Une fixture peut porter sur :

- une règle orthotypographique ;
- une décision structurelle ;
- un invariant du pivot ;
- une canonicalisation ;
- un export ;
- une incohérence volontaire que le système doit diagnostiquer.
...
But : vérifier le contrat Python‑JSON.
...
But : vérifier qu'une décision est inscrite correctement dans le pivot.

Exemples :

- `canonicalize_poetry_sequence_to_quote_block` ;
- `canonicalize_docx_tei_verse_style` ;
- `canonicalize_quote_block_with_line_breaks` ;
- `canonicalize_heading_with_level` ;
- `canonicalize_table_without_flattening`.
...
But : tester un comportement XML ciblé depuis un pivot validé.

Exemples : `xml_simple_chapter`, `xml_chapter_with_notes`, `xml_quote_nested`, `xml_poetry_lg_l`, `xml_bibliography_basic`.

### 3.9 Export consistency

But : vérifier que plusieurs exports représentent le même pivot.
...
- `pivot_notes_docx_xml_agree`.

### 3.10 Rendering HTML / PDF

But : valider le rendu HTML ou PDF d'un XML connu.
...
```text
pivot_contract/<cas>_input.json
pivot_contract/<cas>_expected.json
```
...
Pour la cohérence entre exports :

```text
export_consistency/<cas>_pivot.json
export_consistency/<cas>_expected_docx.txt
export_consistency/<cas>_expected_tei.xml
```
...
Champs canoniques attendus, invariants, diagnostics en cas d'état incomplet.
...
### A. Pivot contract — priorité absolue

1. document minimal sérialisable ;
2. round‑trip JSON stable ;
3. heading avec niveau ;
4. heading sans niveau diagnostiqué ;
5. quote prose ;
6. quote poetry ;
7. protected_zone poetry sans sémantique canonique diagnostiquée ;
8. table conservée ;
9. note liée ;
10. bloc inconnu non exporté silencieusement.

### B. Orthotypographie — priorité absolue
...
### C. Author vs styled — priorité absolue
...
4. citation en bloc ;
5. citation en vers ;
6. entrée bibliographique.

### D. Style suggestions — priorité forte
...
### E. Structure recognition — priorité forte
...
### F. Protected zones — priorité forte
...
### G. XML / rendu — priorité forte

1. chapitre simple XML ;
2. XML avec notes ;
3. XML avec citation prose ;
4. XML avec citation en vers `<lg>/<l>` ;
5. rendu HTML de notes ;
6. rendu HTML de hiérarchie ;
7. rendu PDF de base.
...
### `pivot_contract/poetry_quote_canonical_input.json`

But : vérifier qu'une citation en vers canonique est représentée de façon stable.

Attendu : `block_type=quote_block`, `quote_kind=poetry`, `lineation=verse`, lignes exploitables.

### `pivot_contract/poetry_protected_zone_without_semantics_input.json`

But : vérifier que `protected_zone=poetry` ne suffit pas à produire un export poétique.

Attendu : diagnostic de pivot incomplet ou canonicalisation explicite avant export.
...
### `export_consistency/poetry_docx_xml_agree_pivot.json`

But : vérifier qu'un même pivot poétique est rendu comme vers dans le DOCX et en `<lg>/<l>` dans le XML.

Attendu : aucune heuristique locale différente entre exporteurs.

### `structure_recognition/chronology_not_poetry_input.json`

But : empêcher une chronologie ou une liste datée d'être interprétée comme poésie.

Attendu : pas de `quote_kind=poetry`, diagnostic éventuel, aucun `<lg>/<l>`.
...
Éviter aussi les tests qui valident une sortie visuellement correcte sans vérifier le pivot JSON.

---

## 9. Fiche descriptive recommandée

Chaque famille peut comporter un petit `README.md` local indiquant le but du cas, la source, la règle visée, la raison du choix, les transformations interdites, les invariants du pivot et les diagnostics attendus.
